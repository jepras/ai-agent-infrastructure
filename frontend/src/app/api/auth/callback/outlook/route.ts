...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${backendUrl}/api/auth/callback/outlook?code=${code}&state=${state}`, {
      method: 'GET',
      headers: {
    ...
    return NextResponse.redirect(new URL('/dashboard?oauth_success=true&service=outlook', request.url));
